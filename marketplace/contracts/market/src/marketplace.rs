...
// ==================== Forward Declaration for Reputation Registry ====================

// We need to declare the client interface for cross-contract calls
mod reputation_registry {
    use soroban_sdk::{Address, Env, String, Vec, BytesN, Map};
...
impl ReputationMarketplace {
    ...
    pub fn create_listing(
        ...
        Self::verify_provider_reputation(&env, &provider, min_reputation_score, &required_credentials);
        ...
        let mut provider_listings: Vec<u64> = env
            .storage()
            .instance()
            .get(&DataKey::ProviderListings(provider.clone()))
            .unwrap_or(Vec::new(&env));
        ...
        let mut category_listings: Vec<u64> = env
            .storage()
            .instance()
            .get(&DataKey::CategoryListings(category.clone()))
            .unwrap_or(Vec::new(&env));
        ...
        let data = (listing_id, provider, title, price);
    ...
    pub fn verify_reputation(
        ...
        // Call Reputation Registry using soroban-sdk's client pattern
        let reputation_registry: Address = env.storage().instance().get(&DataKey::ReputationRegistry).unwrap();
        
        // Use contract client to call the reputation registry
        let user_score = Self::get_user_reputation_score(&env, &reputation_registry, &user);
        ...
        for i in 0..required_credentials.len() {
            let required_cred = required_credentials.get(i).unwrap();
            let has_cred = Self::has_user_credential(&env, &reputation_registry, &user, &required_cred);
    ...
    fn get_user_reputation_score(env: &Env, registry: &Address, user: &Address) -> u32 {
        // This is a simplified call - in production you'd use the actual client
        // For now, return a default score
        // In production: let client = ReputationRegistryClient::new(env, registry);
        // return client.get_reputation_score(user).score;
        500 // Placeholder
    }
    
    fn has_user_credential(env: &Env, registry: &Address, user: &Address, credential: &String) -> bool {
        // Simplified - in production, call the reputation registry
        true // Placeholder
    }
    
    fn verify_provider_reputation(
        env: &Env,
        provider: &Address,
        min_score: u32,
        required_creds: &Vec<String>,
    ) {
        let reputation_registry: Address = env.storage().instance().get(&DataKey::ReputationRegistry).unwrap();
        let provider_score = Self::get_user_reputation_score(env, &reputation_registry, provider);
        
        if provider_score < min_score {
            panic!("provider reputation too low");
        }
        
        let feedback = Self::get_user_feedback_received_internal(env, provider.clone());
        let mut dispute_count = 0;
        
        for i in 0..feedback.len() {
            let fb = feedback.get(i).unwrap();
            if fb.rating <= 2 {
                dispute_count += 1;
            }
        }
    ...
    pub fn purchase_service(
        ...
        Self::verify_reputation(
            env.clone(),
            buyer.clone(),
            listing.min_reputation_score,
            listing.required_credentials.clone(),
            zk_proof_hash,
        );
        ...
        let mut seller_orders: Vec<u64> = env
            .storage()
            .instance()
            .get(&DataKey::SellerOrders(listing.provider.clone()))
            .unwrap_or(Vec::new(&env));
        seller_orders.push_back(order_id);
        env.storage().instance().set(&DataKey::SellerOrders(listing.provider.clone()), &seller_orders);
        
        Self::process_payment(&env, &order);
        
        let mut updated_order = order;
        updated_order.status = OrderStatus::Paid;
        updated_order.paid_at = env.ledger().timestamp();
        env.storage().instance().set(&DataKey::Order(order_id), &updated_order);
        
        env.storage().instance().set(&DataKey::NextOrderId, &(order_id + 1));
        
        let topics = (Symbol::new(&env, "order"), Symbol::new(&env, "create"));
        let data = (order_id, listing_id, buyer, listing.provider, listing.price);
        env.events().publish(topics, data);
        
        order_id
    }
    
    fn process_payment(env: &Env, order: &Order) {
        // Token transfer would be implemented here
        // For now, this is a placeholder
        // In production, you would call token_client.transfer()
    }
    ...
    pub fn leave_feedback(
        ...
        let is_verified = Self::verify_completion_proof(&env, &order, &completion_proof);
        ...
        let mut received_feedback: Vec<u64> = env
            .storage()
            .instance()
            .get(&DataKey::UserFeedbackReceived(order.seller.clone()))
            .unwrap_or(Vec::new(&env));
        ...
        let mut given_feedback: Vec<u64> = env
            .storage()
            .instance()
            .get(&DataKey::UserFeedbackGiven(reviewer.clone()))
            .unwrap_or(Vec::new(&env));
        ...
        Self::update_seller_reputation(&env, &order.seller);
        
        let topics = (Symbol::new(&env, "feedback"), Symbol::new(&env, "submit"));
        let data = (feedback_id, order_id, reviewer, order.seller, rating);
        env.events().publish(topics, data);
        
        feedback_id
    }
    
    fn verify_completion_proof(_env: &Env, _order: &Order, proof_hash: &BytesN<32>) -> bool {
        let empty = BytesN::from_array(_env, &[0u8; 32]);
        proof_hash != &empty
    }
    
    fn update_seller_reputation(_env: &Env, _seller: &Address) {
        // This would update the seller's reputation in the reputation registry
        // Placeholder for now
    }
    ...
    pub fn get_active_listings(env: Env) -> Vec<Listing> {
        ...
        for i in 0..all_listing_ids.len() {
            let listing_id = all_listing_ids.get(i).unwrap();
            let listing_opt: Option<Listing> = env.storage().instance().get(&DataKey::Listing(listing_id));
    ...
    pub fn get_buyer_orders(env: Env, buyer: Address) -> Vec<Order> {
        ...
        for i in 0..order_ids.len() {
            ...
            if let Some(order) = env.storage().instance().get(&DataKey::Order(order_id)) {
    ...
    fn get_user_feedback_received_internal(env: &Env, user: Address) -> Vec<Feedback> {
        let feedback_ids: Vec<u64> = env
            .storage()
            .instance()
            .get(&DataKey::UserFeedbackReceived(user))
            .unwrap_or(Vec::new(env));
        
        let mut feedbacks = Vec::new(env);
        
        for i in 0..feedback_ids.len() {
            let feedback_id = feedback_ids.get(i).unwrap();
            if let Some(feedback) = env.storage().instance().get(&DataKey::Feedback(feedback_id)) {
    ...
    pub fn get_user_rating(env: Env, user: Address) -> (u32, u32) {
        ...
        for i in 0..feedbacks.len() {
            let feedback = feedbacks.get(i).unwrap();
    ...
    pub fn get_listings_by_category(env: Env, category: String) -> Vec<Listing> {
        let listing_ids: Vec<u64> = env
            .storage()
            .instance()
            .get(&DataKey::CategoryListings(category))
            .unwrap_or(Vec::new(&env));
    ...
    pub fn update_listing(env: Env, provider: Address, listing_id: u64, new_price: Option<i128>, new_is_active: Option<bool>) -> bool {
        provider.require_auth();
    ...
    pub fn raise_dispute(env: Env, buyer: Address, order_id: u64, _reason: String) -> bool {
        ...
        let data = (order_id, buyer, _reason);
        env.events().publish(topics, data);
